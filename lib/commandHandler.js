...
module.exports = class CommandHandler {
  ...
  async sendEula (eula, message) {
    ...
    try {
      await msg.react('▶️')
      const collected = await msg.awaitReactions((reaction, user) => reaction.emoji.name === '▶️' && user.id === message.author.id, { max: 1, time: 10000, errors: ['time'] })
      ...
      if (reaction.emoji.name === '▶️') {
        ...
        const collected = await msg.awaitReactions((reaction, user) => ['no', 'yes'].includes(reaction.emoji.name) && user.id === message.author.id, { max: 1, time: 10000, errors: ['time'] })
