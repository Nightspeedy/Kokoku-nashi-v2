...
const { CommandLogs, DisabledCommand, Eula, Strings, Member } = require('@lib/models')
...
module.exports = class CommandHandler {
  ...
  async run (commandKey, message, args) {
    try {
      ...
      if (!eula) eula = await Eula.create({ id: message.author.id })
      if (!eula.accepted) return this.sendEula(eula, message)

      const user = await Member.findOne({ id: message.author.id })
      if (!user && !message.author.bot) await Member.create({ id: message.author.id })
  ...
  async sendEula (eula, message) {
    ...
    try {
      await msg.react('▶️')
      const collected = await msg.awaitReactions((reaction, user) => reaction.emoji.name === '▶️' && user.id === message.author.id, { max: 1, time: 60000, errors: ['time'] })
      ...
      if (reaction.emoji.name === '▶️') {
        ...
        const collected = await msg.awaitReactions((reaction, user) => ['no', 'yes'].includes(reaction.emoji.name) && user.id === message.author.id, { max: 1, time: 300000, errors: ['time'] })
      ...
      await msg.edit({
        embed: {
          ...
          description: 'Timed out, Please re-enter your command.',
          timestamp: new Date()
        }
      })
    }
