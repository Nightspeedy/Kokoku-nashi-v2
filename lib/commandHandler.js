...
module.exports = class CommandHandler {
  ...
  async run (commandKey, message, args) {
    try {
      ...
      if (!eula) {
        await Eula.create({ id: message.author.id })
        eula = await Eula.findOne({ id: message.author.id })
      }
  ...
  async sendEula (eula, message) {
    var embed = new RichEmbed()
      ...
      .setDescription('Kokoku is growing, Therefore we have created some Terms of Service, Please accept our TOS before moving on!')
