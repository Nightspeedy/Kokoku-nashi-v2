...
module.exports = class Command {
  ...
  async trigger (message, args, color, bot) {
    ...
    let cool = await this.bot.cooldowns.get(`${this.name}-${message.author.id}`)
    
    if (cool) return message.channel.send("Nope")
    
    if (this.cooldownTime) {
      this.cooldown(this.cooldownTime * 1000, message.author)
    }
