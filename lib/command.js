...
module.exports = class Command {
  ...
  mention(string, message) {
    try {
      let idTag = string.match(/\<\@(.*?)\>/) // Find any string that looks like <@ ... >
      ...
      if(idTag && idTag[1]) { // String is an ID tag
        ...
        mention = bot.users.get(idTag[1]) // Try fetching a user with the ID of the ID tag.
      } else if(!isNaN(string)) { // Is the string just numbers?
        mention = bot.users.get(string) // Try fetching a user with the ID of string.
      } else if (message) {
