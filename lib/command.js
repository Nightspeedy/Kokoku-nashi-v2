...
module.exports = class Command {
  ...
  /**
   * Turns a string into a member object.
  ...
  mention (possibleUser, message) {
    ...
    try {
      const idTag = possibleUser.match(/<@!?([0-9]+)>/) // Find any string that looks like a mention
      ...
      let mentioned = message.guild.members.find(member => member.user.id === IDToCheck) //
      ...
      if (!mentioned && message) {
        var member = message.guild.members.find(member => member.user.tag === possibleUser ||
      ...
      mentioned = member ? member.user : mentioned.user
