...
module.exports = class Command {
  ...
  mention (string, message) {
    try {
      const idTag = string.match(/<@!?([0-9]+)>/) // Find any string that looks like a user mention
      // idTag[0] is the entire tag, idTag[1] is the content.

      let IDToCheck

      if (!isNaN(string)) { // Is the string just numbers?
        IDToCheck = string
      } else if (idTag && idTag[1]) { // String is a tag.
        IDToCheck = idTag[1].replace(/[^0-9]/g, '') // Remove all characters that arent numbers.
      }
      mention = this.bot.fetchUser(IDToCheck) // Try fetching a user with the ID.
      ...
      if (!mention && message) {
        const member = message.guild.members.find(member => member.user.tag === string ||
                                                       member.displayName.toLowerCase() === string.toLowerCase() ||
                                                       member.user.username.toLowerCase() === string.toLowerCase())
        mention = member ? member.user : undefined
      }

      return mention || undefined // Return the user, or undefined if not found
    } catch (e) {
      console.error(e)
