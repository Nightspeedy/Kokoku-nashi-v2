...
module.exports = class Command {
   ...
   * @param {Object} params - Containing the {@link Message} object and, optionally, the args {@link Object}.
   */
  success (title, description, { message, args }) {
    message.channel.send({ embed: {
      color: 0x32e554,
      title: title || 'Scucess!',
   ...
   * @param {Discord.User} user - The title of the embed.
   */
  async cooldown (time, user) {
    await this.main.cooldowns.push(time, `${this.name}-${user.id}`)
  }
