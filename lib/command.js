const { Message, Client, MessageMentions } = require('discord.js') // eslint-disable-line
...
module.exports = class Command {
   ...
   */
  mention (possibleUser, message) {
    try {
      const idTag = possibleUser.match(/<@!?([0-9]+)>/) // Find any string that looks like a user mention
      // idTag[0] is the entire tag, idTag[1] is the content.

      let IDToCheck

      if (!isNaN(possibleUser)) { // Is possibleUser just numbers?
        IDToCheck = possibleUser
      } else if (idTag && idTag[1]) { // String is a tag.
        IDToCheck = idTag[1].replace(/[^0-9]/g, '') // Remove all characters that arent numbers.
      }

      let mentioned = this.bot.users.get(IDToCheck) // Try fetching a cached user with the ID.
      // let mentioned = this.bot.fetchUser(IDToCheck).catch(e => console.error(e))

      // A name could be all-digits!
      // If you didn't find someone by that id, look for people with that name as well.
      // Tough luck for anyone who uses another user's id as name.
      if (!mentioned && message) {
        ...
        mentioned = member ? member.user : undefined
      }

      return mentioned || undefined // Return the user, or undefined if not found
    } catch (e) {
      console.error(e)
