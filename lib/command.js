...
module.exports = class Command {
  ...
  mention (string, message) {
    try {
      const idTag = string.match(MessageMentions.USERS_PATTERN) // Find any string that looks like <@ ... >
      ...
      if (!mention && message) {
        mention = message.guild.members.find(member => member.user.tag === string ||
                                                       member.displayName.toLowerCase() === string.toLowerCase() ||
                                                       member.user.username.toLowerCase() === string.toLowerCase())
      }

      return mention || undefined // If mention not null, return user object. Else, return without modification.
    } catch (e) {
      console.error(e)
